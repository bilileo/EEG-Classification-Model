...
def train_mlp(X_train_selected: np.ndarray, y_train: np.ndarray, X_test_selected: np.ndarray, y_test: np.ndarray, xgb_accuracy: float, y_pred_xgb: np.ndarray) -> None:
    ...
    print("\n" + "=" * 70)
    print("PASO 8: GENERANDO VISUALIZACIONES (Incluyendo PCA)")
    print("=" * 70)

    # 1. Gráficos de barras y matrices de confusión
    ...
    # 2. PCA: Análisis de Componentes Principales para ver Errores vs Aciertos POR CLASE
    print("Generando gráficos PCA por clase...")
    pca = PCA(n_components=2)
    X_test_pca = pca.fit_transform(X_test_selected) # Reducimos a 2D

    # Creamos una figura más ancha para dar espacio a las leyendas
    ...
    for i, (nombre_clase, color) in enumerate(zip(TARGET_NAMES, colores_clases)):
        # ----------------------------------------------------
        # XGBoost - Separación por clase
        # ----------------------------------------------------
        idx_clase = (y_test == i) # Índices donde la clase real es 'i'
        idx_aciertos_xgb = idx_clase & (y_test == y_pred_xgb)
        idx_errores_xgb = idx_clase & (y_test != y_pred_xgb)

        # Graficar Aciertos (Círculos)
        if np.any(idx_aciertos_xgb):
            axes[0].scatter(X_test_pca[idx_aciertos_xgb, 0], X_test_pca[idx_aciertos_xgb, 1],
                            c=color, marker='o', alpha=0.6, label=f'✓ {nombre_clase}')
        
        # Graficar Errores (Tachitas/Cruces grandes con borde)
        if np.any(idx_errores_xgb):
            axes[0].scatter(X_test_pca[idx_errores_xgb, 0], X_test_pca[idx_errores_xgb, 1],
                            c=color, marker='X', s=120, edgecolor='black', linewidths=1.5, 
                            alpha=0.9, label=f'✗ Error (Era {nombre_clase})')

        # ----------------------------------------------------
        # MLP - Separación por clase
        # ----------------------------------------------------
        ...
        if np.any(idx_aciertos_mlp):
            axes[1].scatter(X_test_pca[idx_aciertos_mlp, 0], X_test_pca[idx_aciertos_mlp, 1],
                            c=color, marker='o', alpha=0.6, label=f'✓ {nombre_clase}')
        
        # Graficar Errores (Tachitas/Cruces grandes con borde)
        if np.any(idx_errores_mlp):
            axes[1].scatter(X_test_pca[idx_errores_mlp, 0], X_test_pca[idx_errores_mlp, 1],
                            c=color, marker='X', s=120, edgecolor='black', linewidths=1.5, 
                            alpha=0.9, label=f'✗ Error (Era {nombre_clase})')

    # Configuración final gráfica XGBoost
    axes[0].set_title(f"PCA Errores por Clase - XGBoost")
    axes[0].set_xlabel("Componente Principal 1")
    axes[0].set_ylabel("Componente Principal 2")
    axes[0].legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize='small') # Leyenda por fuera

    # Configuración final gráfica MLP
    axes[1].set_title(f"PCA Errores por Clase - MLP")
    axes[1].set_xlabel("Componente Principal 1")
    axes[1].set_ylabel("Componente Principal 2")
    axes[1].legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize='small') # Leyenda por fuera

    # Ajustar para que las leyendas externas no se recorten
    plt.tight_layout(rect=[0, 0, 0.85, 1]) 
    plt.savefig("pca_errores_modelos.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    print("✓ Gráfico de PCA guardado: pca_errores_modelos.png")

    print("\n" + "=" * 70)
    print("✓ ANÁLISIS COMPLETADO EXITOSAMENTE")
    print("=" * 70)
    winner_name, winner_score = max({"XGBoost": xgb_accuracy, "Red Neuronal MLP": accuracy_mlp}.items(), key=lambda item: item[1])
    print(f"\n ANADOR: {winner_name} con {winner_score:.4f} accuracy")
    print("\n  ARCHIVOS GENERADOS:")
    print("  • rfecv_selection_mlp.png")
    print("  • comparison_mlp.png")
    print("  • pca_errores_modelos.png")
